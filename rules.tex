...
\section{To battle!}
    ...
    \subsection{Production phase: the pulse}
        ...
        The players then place those resources on their territory tiles in any way
        they want.
...
\newpage
    \subsection{Expansion phase}
        ...
        A player's turn goes like this, in this order:
        \vspace{-1.3em}
        \begin{enumerate}
            \item He discards and/or sets aside any number of cards from his hand,
                \label{expand1}
            \item He may start battles on up to 2 frontiers of his territory
                (see section \ref{battle-rules}),
                either against \textit{wild territories}
                ---~which are territories that no player control~---
                or against ones controlled by the same player.
                \label{expand2}
            \item He discards and/or sets aside any number of cards from his hand.
                \label{expand3}
            \item He replenishes his hand.
                If a battle against another player occurred,
                that player may replenish his hand as well.
            \item If he won a battle against another player:
                he draws the cards that he had set aside into his hand,
                then sets aside as many.
            \item If he won a battle against another player:
                he may choose to return to step \ref{expand1},
                with slightly altered steps:
                \vspace{-0.5em}
                \begin{itemize}
                    \item steps \ref{expand1} and \ref{expand3}:
                        the player can discard cards but not set them aside.
                    \item step \ref{expand2}:
                        attacking the same player twice during the same turn
                        is only allowed from newly conquered territories,
                        and does not give the right use step \ref{expand6} again
                        if the defending player's portal is still standing.
                \end{itemize}
                \label{expand6}
            \item He can choose to destroy his own portal.
