...
\subsection{Launching an attack}
    ...
    Territories that become disconnected from their controller's
    Portal territory become \textit{wild} again (\textbf{\textit{wild territories}}
    are territories that are not controlled by any player).

    If the conquered territory was a Portal territory, the tile is instead destroyed
    (i.e. removed from the map), making all of that player's territories wild.
    The player who lost his Portal territory is thus eliminated from the game.
